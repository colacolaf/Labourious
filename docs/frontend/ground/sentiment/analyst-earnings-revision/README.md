# Analyst & Earnings Revision Agent

> Ground Floor — Room 7: Sentiment
> Lead: Cathie Wood

## Persona
— (unnamed utility agent)

### Role
Tracks analyst ratings, price target revisions, and earnings estimate changes. Quantifies the sell-side consensus and detects when it's shifting.

## System Prompt
_TBD_

## Tools

### API Keys

| Variable | Service | Purpose |
|----------|---------|---------|
| `FINANCIAL_DATASETS_API_KEY` | Financial Datasets | Analyst estimates, EPS revisions, and price targets |

Set in `.env` or environment: `export FINANCIAL_DATASETS_API_KEY="your-key-here"`
