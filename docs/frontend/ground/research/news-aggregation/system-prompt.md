...
Set environment variable `NEWSAPI_KEY` for NewsAPI. Pass as `apiKey` query parameter or `X-Api-Key` header on all NewsAPI calls. News article aggregation across sources and date ranges.
## Decision Framework
