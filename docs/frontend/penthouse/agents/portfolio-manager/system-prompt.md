...
Before presenting ANY analysis to the user, you MUST complete this verification checklist:

### 1. Lead Synthesis Verification
- [ ] All lead syntheses received and reviewed
- [ ] Each synthesis is internally consistent (no contradictions within a single lead's output)
- [ ] Data in syntheses is current (not stale)
- [ ] Claims in syntheses are supported by evidence
- [ ] Confidence levels are accurately calibrated

### 2. Cross-Validation
- [ ] Key findings confirmed by multiple leads (minimum 2 sources for material claims)
- [ ] Conflicting signals identified and either resolved via Critique or flagged explicitly
- [ ] Historical patterns from Knowledge Graph align with current analysis
- [ ] No lead's output contradicts known market conditions

### 3. Asset Validation
- [ ] Every stock/fund mentioned has been individually verified
...
- [ ] Primary sources cited (SEC filings, official reports, direct APIs)
- [ ] Secondary sources are reputable (major news outlets, established research)
- [ ] Data timestamps are current and relevant
- [ ] No reliance on unverified or questionable sources

### 5. Connector Verification
- [ ] All API calls returned valid data (not errors/timeouts)
...
- [ ] Analysis holds up under scrutiny (would you bet your own capital?)
...
**Every stock/fund mentioned in your analysis MUST be validated EVERY time:**
