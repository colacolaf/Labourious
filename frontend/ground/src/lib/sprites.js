// ---------------------------------------------------------------------------
// sprites.js — desksprite EXACT pixel routines, ported 1:1 from deskrpg's
//               src/game/scenes/BootScene.ts (drawTile function, lines 25–293).
//
// Every hex color, every fillRect coordinate, every fillStyle alpha is copied
// verbatim from deskrpg so this looks pixel-identical to a real deskrpg map.
// ---------------------------------------------------------------------------

export const TILE = 32;

// Tile-IDs (must match deskrpg's `T` enum in GameScene.ts line 87):
//   0: empty/transparent   1: floor              2: wall              3: desk
//   4: chair               5: computer screen    6: plant             7: door
//   8: meeting table       9: coffee area       10: water cooler     11: bookshelf
//  12: carpet             13: whiteboard        14: reception desk   15: cubicle wall
export const TILE_ID = Object.freeze({
  EMPTY: 0, FLOOR: 1, WALL: 2, DESK: 3, CHAIR: 4, COMPUTER: 5, PLANT: 6,
  DOOR: 7, MEETING_TABLE: 8, COFFEE: 9, WATER_COOLER: 10, BOOKSHELF: 11,
  CARPET: 12, WHITEBOARD: 13, RECEPTION_DESK: 14, CUBICLE_WALL: 15,
});

// ---------------------------------------------------------------------------
// drawTile — verbatim port of deskrpg's BootScene.drawTile().
// ---------------------------------------------------------------------------
export function drawTile(g, index) {
  const x = index * TILE;
...
// ---------------------------------------------------------------------------
// buildOfficeTileStrip — port of deskrpg's BootScene.create() lines 300–309.
// Produces one horizontal strip of 16 frames, each 32×32 px.
// ---------------------------------------------------------------------------
export function buildOfficeTileStrip(scene) {
  const graphics = scene.add.graphics();
  for (let i = 0; i < 16; i++) drawTile(graphics, i);
  graphics.generateTexture("office-tiles", 16 * TILE, TILE);
  graphics.destroy();

  // Register each 32-wide cut-out as an individual frame.
  const tex = scene.textures.get("office-tiles");
  for (let i = 0; i < 16; i++) {
